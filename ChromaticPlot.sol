...
contract ChromaticPlot is IERC721 {
    ...
    //////////////////////////
    bool isMainnet = true;
    address public mappedContractAddress;

    function setMappedContract(address _mappedContractAddress)
        external
        onlyAdmin
    {
        require(networkId != 0, "Cannot be mainnet contract");
        mappedContractAddress = _mappedContractAddress;
        isMainnet = false;
    }

    function mintWithTickets(uint256[] calldata _ids) external reentrancyGuard {
        require(!isMainnet, "Cannot be mainnet contract");
        for (uint256 i = 0; i < _ids.length; i++) {
            _mintWithTicket(_ids[i]);
        }
    }

    function _mintWithTicket(uint256 _id) internal {
        require(IERC721(mappedContractAddress).ownerOf(_id) == msg.sender);
        IERC721(mappedContractAddress).transferFrom(
            msg.sender,
            address(this),
            _id
        );
        _mappedMint(msg.sender, _id);
    }

    function _mappedMint(address _to, uint256 _id) internal returns (uint256) {
        require(_to != address(0), "Cannot mint to 0x0.");
        numTokens = numTokens + 1;
        _addNFToken(_to, _id);
        emit Mint(_id, _to, false);
    ...
    function mint(uint256 _quantity) external payable reentrancyGuard {
        require(isMainnet, "Sale not started.");
        require(publicSale, "Sale not started.");
        require(
    ...
    function _getTransmuterNFTCount(address _transmuter)
        internal
        view
        returns (uint256)
    {
        return transmuterToIds[_transmuter].length;
    }
    ...
    function transmutingBalanceOf(address _transmuter)
        external
        view
        returns (uint256)
    {
        require(_transmuter != address(0));
        return _getTransmuterNFTCount(_transmuter);
    }
    ...
    function transmuteChromaNFT(uint256 _plotId, uint256 _count) external {
        ...
        ChromaInterface(chroma).transmutePlotToNFTs(
            msg.sender,
            _plotId,
            _count
        );
    ...
    function transmuteChromaERC20(uint256 _plotId, uint256 _count) external {
        ...
        ChromaInterface(chroma).transmutePlotToERC20(
            msg.sender,
            _plotId,
            _count
        );
    ...
    function getTransmutingTokenIdsSegment(
        ...
        uint256[] memory transmuting = new uint256[](count);
        for (uint256 i = startIndex; i < startIndex + count; i++) {
            transmuting[i - startIndex] = transmuterToIds[owner][i];
        }
        return transmuting;
    }
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(
                    "https://chromatic.art/api/metadata?type=plot&id=",
                    toString(_tokenId),
                    "&network=",
                    toString(networkId)
                )
