...
contract ChromaticCanvas is IERC721 {
    ...
    event TitleChanged(uint256 indexed canvasId, string title);
    event ArtistChanged(uint256 indexed canvasId, string artist);

    uint256 public tokenIndex = 0;

    mapping(uint256 => string) public idToTitle;
    mapping(uint256 => string) public idToArtist;
    ...
    function changeTitle(uint256 _canvasId, string calldata _title)
        external
        onlyCanvasOwner(_canvasId)
        notLocked(_canvasId)
    {
        idToTitle[_canvasId] = _title;
        emit TitleChanged(_canvasId, _title);
    }

    function changeArtist(uint256 _canvasId, string calldata _artist)
        external
        onlyCanvasOwner(_canvasId)
        notLocked(_canvasId)
    {
        idToArtist[_canvasId] = _artist;
        emit ArtistChanged(_canvasId, _artist);
    }
    ...
    function createCanvas(
        ...
        idToTitle[id] = _title;
        idToArtist[id] = _artist;
        idToDimensions[id] = [_height, _width];
        // idToHeight[id] = _height;
        // idToWidth[id] = _width;
        emit CanvasCreated(id, _baseChromaId, _height, _width, _title, _artist);
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(
                    "https://chromatic.art/api/metadata?type=canvas&id=",
                    toString(_tokenId),
                    "&network=",
                    toString(networkId)
                )
